...
exports.verifyGroupOrderPayment = async (req, res) => {
  try {
    ...
    // Verify transaction matches razorpay_order_id
    if (transaction.razorpayOrderId !== razorpay_order_id) {
      return res.status(400).json({
        success: false,
        message: 'Transaction does not match Razorpay order ID',
      });
    }
    ...
    if (!order.groupOrderId) {
      return res.status(400).json({
        success: false,
        message: 'Order is not associated with a group order',
      });
    }
    order.status = 'placed';
    order.paymentStatus = 'paid';
    ...
    // Update transaction status in groupOrder.paymentDetails.transactions
    groupOrder.paymentDetails.transactions = groupOrder.paymentDetails.transactions.map((txn) =>
      txn.transactionId.toString() === transaction._id.toString()
        ? { ...txn, status: 'paid' }
        : txn
    );

    // Check if all transactions are paid
    const allTransactionsPaid = groupOrder.paymentDetails.transactions.every(
      (txn) => txn.status === 'paid'
    );

    if (allTransactionsPaid) {
      groupOrder.status = 'completed';
      // Update all associated Orders to 'completed'
      ...
      for (const member of groupOrder.members) {
        await SendNotification(
          ...
          `Group order ${groupOrder.orderNumber} is now completed!`
        );
      }
    }
    ...
    // Populate and broadcast the updated GroupOrder
    ...
    res.status(200).json({
      success: true,
      message: 'UPI payment for group order verified successfully',
      data: {
        transactionId: transaction._id,
        orderId: order._id,
